...
export default function ResultPageContent({
  ...
  initialType?: string;
  initialPayload?: SharePayloadV1 | null;
}) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const {
    dogName: storeDogName,
    ownerName: storeOwnerName,
    breedId: storeBreedId,
    ownerMbti: storeOwnerMbti,
    photoUrl: storePhotoUrl,
    answers,
    reset,
    saveToHistory,
    setPhotoUrl,
  ...
  const shared = useMemo(
    () => initialPayload ?? decodeSharePayload(searchParams.get("d")),
    [initialPayload, searchParams]
  );
  const type = shared?.t || initialType || "";

  const dogName = shared?.d || storeDogName;
  const ownerName = shared?.o || storeOwnerName;
  const breedId = shared?.b || storeBreedId;
  const ownerMbti = shared?.m || storeOwnerMbti;
  const photoUrl = storePhotoUrl; // 사진은 공유 불가 (blob URL)

  const result = resultData[type];
  const scores = computeScores(answers);
  const percentages = shared?.p || getAxisPercentages(scores);
  const breed = breeds.find((b) => b.id === breedId);
  const ownerMatch = ownerMatches[type];

  const [step, setStep] = useState(shared ? 4 : 0); // 공유 링크면 바로 최종 결과
  ...
  const shareUrl = useMemo(() => {
    ...
    return `${window.location.origin}/result?d=${payload}`;
